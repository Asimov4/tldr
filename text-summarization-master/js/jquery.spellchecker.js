...
(function(window, $) {
  ...
  var removeUncessaryWordsRules = {
		// http://www.plainlanguage.gov/howto/guidelines/FederalPLGuidelines/writeOmitUnnecc.cfm
		rules: [
			{
			regex: /a number of/gi,
			needle: "a number of",
			replace: ["several"]
			},
			{
			regex: /a sufficient number of/gi,
			needle: "a sufficient number of",
			replace: ["enough"]
			},
			{
			regex: /at this point( in time)?/gi,
			needle: "at this point( in time)?",
			replace: ["now"]
			},
			{
			regex: /on an? ([a-z]+) basis/gi,
			needle: "on an? [a-z]+ basis",
			replace: ["$1"]
			},
			{
			regex: /on the ground that/gi,
			needle: "on the ground that",
			replace: ["because"]
			},
			{
			regex: /an amount of ([0-9]+)/gi,
			needle: "an amount of [0-9]+",
			replace: ["$1"]
			},
			{
			regex: /in order to/gi,
			needle: "in order to",
			replace: ["to"]
			},
			{
			regex: /different/gi,
			needle: "different",
			replace: ["You could probably remove that"]
			},
			{
			regex: /currently/gi,
			needle: "currently",
			...
			{
			regex: /the reason why/gi,
			needle: "the reason why",
			replace: ["why"]
			},
			{
			regex: /(who|that) (is|are)/gi,
			needle: "(who|that) (is|are)",
			replace: ["remove","$2"]
			},
			{
			regex: /or not/gi,
			...
			{
			regex: /the reason is that/gi,
			needle: "the reason is that",
			replace: ["You could probably remove that"]
			}
	...
	function removeUnnecessaryWords(text,removeUncessaryWordsRules) {

		var suggestions = [];
		$.each(removeUncessaryWordsRules, function(index, rule) {

			$.each(rule, function(index, item) {
				if (item.regex.test(text)) {
					$.each(item.replace, function(index, replacement) {
						suggestions.push(text.replace(item.regex,replacement));
					});
				}
			});
		});
  ...
  WebService.prototype.checkWords = function(text, callback) {
	...
	var words = new Lexer().lex(text[0]);
	...
	callback({"outcome":"success","data":[errors]});
  };

  WebService.prototype.getSuggestions = function(word, callback) {
	var suggest = removeUnnecessaryWords(word,removeUncessaryWordsRules);
	if (suggest.length == 0) {
		suggest.push("You could probably remove this adverb.");
	}
    callback(suggest);
  };
  ...
  HtmlParser.prototype.highlightWords = function(incorrectWords, element) {
    ...
    this.replaceText(new RegExp(regExp, 'gi'), element[0], this.highlightWordsHandler(), 2);
  };
